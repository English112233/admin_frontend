<template>
    <div id="view">
        ...
        <table class="table mt20 table-striped table-bordered table-hover" border="1" cellspacing="0" cellpadding="0"
            ...
            <!--清算银行卡-->
            <template v-if="merchantLevel==='second'">
                <tr class="division-msg">
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            merchantId: [String, Number] // 二级商户id
        },
        beforeMount: function () {
            this.$getJson({
                ...
                callback: function (vue, res) {
                    ...
                    vue.checking = res.content.infoStatusChecking === res.content.merchantInfo.infoStatus
                    vue.checkFail = res.content.infoStatusFail === res.content.merchantInfo.infoStatus
                    vue.info = res.content.merchantInfo
                    vue.statusMap = res.content.infoStatusOptions

                    vue.bankAccount = res.content.bankAccount || {}// 清算银行
                    vue.legalPerson = res.content.legalPerson || {} // 法人
                    vue.owners = res.content.owners || [] // 受益人

                    vue.legalPerson && (vue.dateInitFn(vue.legalPerson)) // 初始化法人证件有效期
                    vue.owners && vue.owners.map((sun) => vue.dateInitFn(sun)) // 初始收益人证件有效期
                }
            })
        },
        computed: {
